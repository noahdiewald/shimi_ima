function(newDoc, saveDoc, userCtx) {
  ...
  var documents = function(newDoc, saveDoc, userCtx) {
    ...
    // field.
    var validationTests = [requiredField, dateField, booleanField, 
                           docmultiselectField, docselectField, integerField, 
                           multiselectField, openbooleanField, rationalField, 
  ...
  var userIndex = function(newDoc, saveDoc, userCtx) {
    ...
    if (newDoc.conditions && newDoc.conditions.length > 0) {
      ...
      if (firstCondition.parens === 'close' ||
          firstCondition.parens === 'exclose' ||
          lastCondition.parens === 'exopen' ||
          lastCondition.parens === 'open') {
        forbid('Conditions', 
               'begin or end with improper parenthesis.');
      }
      ...
      newDoc.conditions.forEach(
        function(condition, index) {
          ...
          if (exParenCount > 1) {
            forbid('Condition ' + index,
                   'existential parenthesis cannot be nexted.');
          }
          ...
          if (!parens && exParenCount === 1) {
            if (exParenFS) {
              if (exParenFS !== fieldset && !is_or) {
                forbid('Condition ' + index, 
                       'all conditions within existential scope must have ' + 
                       'the same fieldset.');
              }
            } else {
              exParenFS = fieldset;
